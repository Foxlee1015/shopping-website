...
def home():
    ...
    try:
        ...
        email = session['email']
        points = select_data(table_name="user_list", select_column="points", column1="email", row=str(email))
...
def order_list():
    form = Submit_Form(request.form)
    email = session['email']
    user_id = select_data(table_name="user_list", column1="email", row=email)[0][0]
