...
class LoginForm(Form):
    email = TextAreaField('Email Address', [validators.data_required(), validators.Length(min=6, max=50)])
    password = PasswordField('Password', [validators.data_required()])
    submit = SubmitField('Login')

class RegistrationForm(Form):
    username = TextAreaField('Username', [validators.data_required(), validators.Length(min=4, max=20)])
    email = TextAreaField('Email Address', [validators.data_required(), validators.Length(min=6, max=50)])
    password = PasswordField('Password', [validators.data_required(),
